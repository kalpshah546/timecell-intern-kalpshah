...
- Pre-computed risk metrics (post-crash value, runway in months, ruin test,
  largest risk asset, concentration warning) for severe and moderate crash scenarios
- Recent news headlines (last 7 days) for each asset
...
- Reference news ONLY in the news_insight field, unless news directly affects risk.
- If no news is available for an asset, the news_insight may say so.
...
def build_user_message(
    ...
    """Bundle portfolio + computed metrics + news into a clean user message."""
    news_section = ""
    if news_bundle:
        news_section = (
            "\n\nRECENT NEWS HEADLINES (LAST 7 DAYS):\n"
            f"{json.dumps(news_bundle, indent=2)}"
        )

    return f"""Here is the client's portfolio and supporting data:

PORTFOLIO:
{json.dumps(portfolio, indent=2)}

RISK METRICS (severe = full expected crash, moderate = half crash):
{json.dumps(risk_metrics, indent=2)}{news_section}
